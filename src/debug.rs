    ...
    let rect_str = match &node.content {
        crate::layout::LayoutNodeContent::TextLayoutNode(text_node) => {
            ...
            for css_text_box in text_node.css_text_boxes.iter() {
                rect_str.push_str(format!("CssTextBox({:?} {:?})", css_text_box.css_box, css_text_box.text).as_str());
            };
            rect_str
        },
        ...
        crate::layout::LayoutNodeContent::AreaLayoutNode(area_layout_node) => { format!("AreaLayoutNode({:?})", area_layout_node.css_box) }
